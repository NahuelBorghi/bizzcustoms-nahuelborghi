...
export const ItemDetail = ({article:{colors,id,imagesId,name,price,stock,type}}) => {
  ...
  return (
    <>
      {name != undefined? (
        <>
        <div id="imgProduct"><img src={require(`../../assets/images/productos/${imagesId[0]}`).default}/></div>
        <div id="datos">
        ...
        <div id="cartButton">
          <button id="toCart" onClick={()=>{if(style>0){add()}}} style={{margin:"10%",opacity:style}}><Link exact to="/Cart">Terminar compra</Link></button>        
        </div>
      </>):(
        <>
          <Loading/>
